...
def convert_to_onnx(model, rgb_camXs, pix_T_cams,cam0_T_camXs, vox_util,rad_occ_mem0, device):
    ...
    class ONNXWrapper(torch.nn.Module):
        ...
        def forward(self, rgb_camXs, pix_T_cams, cam0_T_camXs, rad_occ_mem0):
            # Pass all inputs explicitly
            outputs = self.model(
                rgb_camXs=rgb_camXs,
                pix_T_cams=pix_T_cams,
                cam0_T_camXs=cam0_T_camXs,
                rad_occ_mem0=rad_occ_mem0,  # Ensure it is explicitly passed
                vox_util=self.vox_util       # This can remain as an internal attribute
            )
            return outputs
    ...
    wrapper.eval() 
    onnx_filename = "simple_bev_op20_eval.onnx"
